                            ...
                            stateSize:(NSUInteger *)stateSize
{
    ...
    [result setLength:((paddingBits == 0) ? (HIDDescriptorSizeWithoutPadding) : (HIDDescriptorSizeWithPadding))];
    ...
    *data = 0x05;
    data++;
    *data = 0x09;
    data++; //  USAGE_PAGE (Button)
    *data = 0x19;
    data++;
    *data = 0x01;
    data++; //  USAGE_MINIMUM (Button 1)
    *data = 0x29;
    data++;
    *data = buttonCount;
    data++; //  USAGE_MAXIMUM (Button buttonCount)
    *data = 0x15;
    data++;
    *data = 0x00;
    data++; //  LOGICAL_MINIMUM (0)
    *data = 0x25;
    data++;
    *data = 0x01;
    data++; //  LOGICAL_MAXIMUM (1)
    *data = 0x95;
    data++;
    *data = buttonCount;
    data++; //  REPORT_COUNT (buttonCount)
    *data = 0x75;
    data++;
    *data = 0x01;
    data++; //  REPORT_SIZE (1)
    ...
    *data = 0x75;
    data++;
    *data = paddingBits;
    data++; //  REPORT_SIZE (paddingBits)
    *data = 0x81;
    data++;
    *data = 0x03;
    data++; //  INPUT (Cnst, Var, Abs)
